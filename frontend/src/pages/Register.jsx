...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message = err.response?.data?.message || err.message || 'Registration failed - check connection';
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors duration-500">
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-96 h-96 bg-emerald-500/20 rounded-full blur-[100px] animate-pulse"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-indigo-500/20 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '2s' }}></div>
      </div>
      ...
      <div className="max-w-md w-full space-y-8 glass p-10 rounded-3xl z-10 animate-in fade-in zoom-in duration-500">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Join your team on CollabSphere
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div className="relative group">
              ...
              <input
                ...
                type="text"
                required
                className="appearance-none relative block w-full px-3 py-3 pl-10 border border-gray-300 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 placeholder-gray-500 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:z-10 sm:text-sm transition-all shadow-sm"
